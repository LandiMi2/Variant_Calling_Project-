...
rule mapping:
        ...
        shell:
            "bwa mem -t {threads} {params.index} {input.read1} {input.read2} > {output} "
...
rule removal_deduplication:
        ...
        shell:
            "sambamba markdup -r {params.sam_index} {output}"
...
rule Create_Ref_Dictonary: 
        ...
        output:
            "Data/chr1_ref.dict"
        shell:
            "gatk CreateSequenceDictionary -R {input[0]} -O {output}"
...
rule AddOrReplaceReadGroups: 
        ...
        shell:
            "gatk AddOrReplaceReadGroups -I {input[0]} -O {output} -ID 4 -LB lib1 -PL illumina -PU unit1 -SM 20"
...
rule BaseRecalibrator:
        ...
        shell:
            "gatk BaseRecalibrator -R {input[0]} -I {input[1]} -O {output} --known-sites {input[2]}"
...
rule ApplyBQSR :
        ...
        shell:
            "gatk ApplyBQSR -bqsr {input[0]} -I {input[1]} -O {output}"
...
rule HaplotypeCaller:
        ...
        shell:
            "gatk HaplotypeCaller -I {input[0]} -O {output} -R {input[1]}"
